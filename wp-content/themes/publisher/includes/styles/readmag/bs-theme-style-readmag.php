 ...
 * Publisher -> Read Mag
 */
class Publisher_Theme_Style_readmag extends Publisher_Theme_Style {
	...
	function panel_std( $fields ) {
		...
		include PUBLISHER_THEME_PATH . 'includes/styles/readmag/panel-std.php';
	...
	function register_assets() {
		...
		bf_enqueue_style(
			'publisher-theme-readmag',
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_uri( 'readmag/style' ), '.css' ),
			array( 'publisher' ),
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_path( 'readmag/style' ), '.css' ),
	...
	public function register_tinymce_assets() {
		...
		bf_enqueue_tinymce_style( 'registered', 'publisher-theme-readmag' );
	}
