...
class simple_html_dom_node {
	...
	function dump( $show_attr = true, $deep = 0 ) {
		...
		echo $lead . $this->tag;
		if ( $show_attr && bf_count( $this->attr ) > 0 ) {
			echo '(';
	...
	function dump_node( $echo = true ) {
		...
		$string .= " children: " . bf_count( $this->children );
		$string .= " nodes: " . bf_count( $this->nodes );
	...
	function last_child() {
		...
		if ( ( $count = bf_count( $this->children ) ) > 0 ) {
	...
	function next_sibling() {
		...
		$idx   = 0;
		$count = bf_count( $this->parent->children );
	...
	function find( $selector, $idx = NULL, $lowercase = false ) {
		...
		for ( $c = 0; $c < $count; ++ $c ) {
			...
			if ( ( $levle = bf_count( $selectors[ $c ] ) ) === 0 ) {
				return array();
			}
...
class simple_html_dom {
	...
	protected function remove_noise( $pattern, $remove_tag = false ) {
		...
		for ( $i = $count - 1; $i > - 1; -- $i ) {
			$key = '___noise___' . sprintf( '% 5d', bf_count( $this->noise ) + 1000 );
			if ( is_object( $debugObject ) ) {
