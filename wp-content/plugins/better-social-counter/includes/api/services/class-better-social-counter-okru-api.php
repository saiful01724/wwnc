...
class Better_Social_Counter_OkRu_API implements Better_Social_Counter_Service_Interface {

	/**
	 * @var string
	 */
	protected $service_id = 'ok_ru';
	...
	public function count() {
		...
		if ( ! $count ) {
			...
			preg_match( "'\<\s*span.*?class\=\"navMenuCount\".*?id=\"groupMembers.*?\".*?\>.*?([0-9\,\.]+).*?\<\/span.*?>'isx", $content, $match );
